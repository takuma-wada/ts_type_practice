...
function combine(input1: number | string, input2: number | string): number | string //戻り値も指定可能
{
    let result;
...
let literalType: 'johnDoe';
// literalType = '100'; => エラーになる
let unionLiteralType: 'john' | 'Doe'; // union型との組み合わせ
unionLiteralType = 'john';
unionLiteralType = 'Doe';
// unionLiteralType = 'johnDoe'; => エラーになる

console.log('----- 型エイリアス -----'); // 型に値を指定するイメージ
type stringAlias = string | number;

//使いまわせる
let type1: stringAlias;
let type2: stringAlias;
